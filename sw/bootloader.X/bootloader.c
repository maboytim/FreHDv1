...
void do_crc16(void)
{
asm("	movlw	low ("___mkstr(BOOTLOADER_END)"+1)");
asm("	movwf	TBLPTRL");
asm("	movlw	high ("___mkstr(BOOTLOADER_END)"+1)");
asm("	movwf	TBLPTRH");
asm("	movlw	low (("___mkstr(BOOTLOADER_END)"+1) shr 16)");
asm("	movwf	TBLPTRU");
asm("");
asm("	clrf	(_crc+0)&0xFF,c");
asm("	clrf	(_crc+1)&0xFF,c");
asm("doc0:	tblrd*+");
asm("	movf	TABLAT,w");
asm("	rcall	crc16");
asm("	movf	TBLPTRU,w		; stop when TBLPTR = 01FFFE");
asm("	sublw	low("___mkstr(FLASH_CRC_ADDR)" shr 16)");
asm("	bnz	doc0");
asm("	movf	TBLPTRH,w		; stop when TBLPTR = 01FFFE");
asm("	sublw	high("___mkstr(FLASH_CRC_ADDR)")");
asm("	bnz	doc0");
asm("	movf	TBLPTRL,w");
asm("	sublw	low("___mkstr(FLASH_CRC_ADDR)")");
asm("	bnz	doc0");
asm("	retlw	0");
//
...
//
asm("crc16:");
asm("	xorwf	(_crc+0)&0xFF,w,c		; W = input xor old_crc+0");
asm("	xorwf	(_crc+1)&0xFF,w,c		; swap old_crc+1 with w");
asm("	xorwf	(_crc+1)&0xFF,f,c");
asm("	xorwf	(_crc+1)&0xFF,w,c		; new crc+1 = input xor old_crc+0");
asm("	movwf	(_crc+0)&0xFF,c		; new crc+0 = old_crc+1");
asm("");
asm("	movf	(_crc+1)&0xFF,w,c		; save crc+1 in W");
asm("	swapf	(_crc+1)&0xFF,f,c		; trade nibbles");
asm("	xorwf	(_crc+1)&0xFF,f,c		; XOR high half byte with low");
asm("	rrcf	(_crc+1)&0xFF,f,c		; initialize carry");
asm("	btfsc	(_crc+1)&0xFF,0,c");
asm("	btg	STATUS,0	; compliment carry");
asm("	btfsc	(_crc+1)&0xFF,1,c");
asm("	btg	STATUS,0	; compliment carry");
asm("	btfsc	(_crc+1)&0xFF,2,c");
asm("	btg	STATUS,0	; compliment carry");
asm("	movwf	(_crc+1)&0xFF,c		; restore crc+1 from W");
asm("");
asm("	movlw	0x1");
asm("	btfsc	STATUS,0	; if carry");
asm("	xorwf	(_crc+0)&0xFF,f,c		; flip bit 0 of crc+0");
asm("	movlw	0x40");
asm("	rrcf	(_crc+1)&0xFF,f,c		; shift parity into crc+1");
asm("	btfsc	STATUS,0	; if shift out is one");
asm("	xorwf	(_crc+0)&0xFF,f,c		; flip bit 6 of crc+0");
asm("	rlcf	(_crc+1)&0xFF,w,c		; unshift crc+1 into W");
asm("	xorwf	(_crc+1)&0xFF,f,c		; combine them");
asm("	rrcf	(_crc+1)&0xFF,f,c		; shift parity back into crc+1");
asm("	movlw	0x80");
asm("	btfsc	STATUS,0	; if shift out is one");
asm("	xorwf	(_crc+0)&0xFF,f,c		; flip bit 7 of crc+0");
}
#endif
