...
const UCHAR (*const trs_extra[])(UCHAR) = {
	trs_extra_version,		// 0
	trs_extra_gettime,		// 1
	trs_extra_settime,		// 2
...
UCHAR trs_extra_openfile(UCHAR step)
{
	if (step == 1) {	
		...
		if (state_error2 != FR_OK) {
			...
			memcpy(extra_buffer, (const void *)&state_file2.obj.objsize,
              sizeof (DWORD));

			{
				...
				// < 2 or -1 means error
				sect = clst2sect(state_file2.obj.fs, state_file2.obj.sclust);
				// 0 means error
...
UCHAR trs_extra_infodrive(UCHAR step)
{
	...
	if (step == 0) {
		...
		for (drive_num = 0; drive_num < TRS_HARD_MAXDRIVES; drive_num++) {
			...
			if(action_type & 0x10)
			{
				memcpy(&extra_buffer[state_size2], (const char *)d->filename, FF_LFN_BUF + 1);
				state_size2 += FF_LFN_BUF + 1;
			}
			else
			{
