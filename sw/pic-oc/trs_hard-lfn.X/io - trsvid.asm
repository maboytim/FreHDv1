...
;;;
	...
	global	_state_busy, _val_1F, _expected_addr, _data2_out, _data2_out1, _data2_stream, _foo
...
handle_int2:
	...
	movf	TRS_ADDR,w,c		;<1> get I/O access address
	cpfseq	BANKMASK(_expected_addr),c		;<1> are we expecting this?
	bra	general_case		;<1> no? handle it generally
	clrf	TRS_DATA_TRIS,c		;<1> TRS_DATA was set, let it go
	bsf	TRS_WAIT,c		;<1> and clear off the wait states
	...
	movff	SPI1RXB,_data2_out
	BANKSEL(SPI1CON2)			; start next read
...
;	movwf	SPI1CON2,b
	...
	movff	_data2_out,TRS_DATA_OUT

	bsf	BANKMASK(_gled_val),7,c		; gled_val |= LED_FLASH (indicate streaming)
...
general_case:
	...
	incf	BANKMASK(_expected_addr),w,c
	bz	not_fast_pending

	btfss	PIR3,0,c
	bra	$-2

	movff	SPI1RXB,_data2_out1
	bsf	SD_CS,c

	movlw	0xff
	movwf	BANKMASK(_expected_addr),c	; invalidate fast case
not_fast_pending:
...
trs_read_data2_slow:
	btfss	BANKMASK(_data2_stream),0,c
	bra	trs_read_data2
	movff	_data2_out,TRS_DATA_OUT
	clrf	TRS_DATA_TRIS,c
	bsf	TRS_WAIT,c
	movf	TRS_ADDR,w,c
	andlw	0x1F
	movwf	BANKMASK(_expected_addr),c	; ensure expected address correct
	...
	bcf	SD_CS,c
	BANKSEL(SPI1CON2)
	movlw	0x02		; set fast clock
	movwf	SPI1BAUD,b

	movff	_data2_out1,_data2_out
	movlw	(SPI1CON2_SPI1RXR_MASK | SPI1CON2_SPI1TXR_MASK)
	movwf	SPI1CON2,b
	movlw	1
	movwf	SPI1TCNTL,b
	movlw	255
	movwf	SPI1TXB,b		; start next read
