...
//#include "../trs_hard.h"
#include "../led.h"
#if UART_DEBUG_FS || PROTEUS_SIMULATOR
#include "../serial.h"
#endif
...
#if 1 // 64MHz
#define	FCLK_SLOW()	(SPI1BAUD = 0x2f)	/* Set slow clock (100k-400k) */
#define	FCLK_FAST()	(SPI1BAUD = 0x02)	/* Set fast clock (depends on the CSD) */
#else
#define	FCLK_SLOW()	(SPI1BAUD = 0x1f)	/* Set slow clock (100k-400k) */
#define	FCLK_FAST()	(SPI1BAUD = 0x01)	/* Set fast clock (depends on the CSD) */
#endif
...
extern __near BYTE crc_enabled;
...
//#define rcvr_spi_m(p)	SPI1TXB = 0xFF; while (!PIR3bits.SPI1RXIF); *(p) = (BYTE)SPI1RXB;
...
BYTE xchg_spi (BYTE dat)
{
	...
	__nop();
	//Return received data from receive buffer register 
	return SPI1RXB;
}
...
)
{
	...
	if (cmd & 0x80) {					/* ACMD<n> is the command sequence of CMD55-CMD<n> */
		cmd &= 0x7F;
		res = send_cmdx(CMD55, 0);
		if (res > 1) return res;
	}

	return send_cmdx(cmd, arg);
}
